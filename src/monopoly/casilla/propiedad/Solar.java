...
public class Solar extends Propiedad {
    ...
    public boolean alquiler() {
        // Este método calcula si se puede cobrar alquiler
    ...
    public String toString() {
        return String.format("Solar{nombre='%s', posicion=%d, valor=%,.0f€, grupo=%s}",
                nombre, posicion, valor, grupo != null ? grupo.getColorGrupo() : "null");
    }

    // MÉTODOS ESPECÍFICOS de Solar requeridos por el PDF
    public void edificar(String tipoEdificio) {
        switch (tipoEdificio.toLowerCase()) {
            case "casa":
                if (anhadirCasa()) {
                    System.out.println("Casa construída en " + this.nombre);
                }
                break;
            case "hotel":
                if (anhadirHotel()) {
                    System.out.println("Hotel construído en " + this.nombre);
                }
                break;
            case "piscina":
                if (anhadirPiscina()) {
                    System.out.println("Piscina construída en " + this.nombre);
                }
                break;
            case "pista_deporte":
                if (anhadirPistaDeporte()) {
                    System.out.println("Pista de deporte construída en " + this.nombre);
                }
                break;
        }
    }
    ...
    public void hipotecar() {
        if (puedeHipotecar(this.duenho)) {
            super.hipotecar();
            System.out.println(this.duenho.getNombre() + " ha hipotecado " + this.nombre + " por " +
                    String.format("%,.0f", this.valorHipoteca) + "€");
        }
    }
    ...
    // MÉTODOS de evaluación de casilla - Polimorfismo
    @Override
    public boolean evaluarCasilla(Jugador actual, Jugador banca, int tirada) {
        if (actual.getAvatar().getLugar() == this) {
            ...
            if (this.duenho == null || this.duenho == banca || this.duenho.getNombre().equals("Banca")) {
                System.out.println("¡Este solar está disponible para compra! Usa el comando 'comprar " + this.nombre + "' para adquirirla.");
                return true;
            }
            ...
            if (this.duenho != null && this.duenho != banca && this.duenho != actual) {
                if (this.hipotecada) {
                    System.out.println("El solar " + this.nombre + " está hipotecado. No se cobra alquiler.");
                    return true;
                }
    ...
    private float calcularAlquilerTotal() {
        float alquilerTotal = this.impuesto; // Alquiler base

        // Sumar alquileres de edificios
        if (this.numCasas > 0) {
        ...
        // Regla del doble por grupo completo (sin edificios)
    ...
    public void infoCasilla() {
        ...
        System.out.println(String.format("\tAlquiler: %,.0f€", this.impuesto));
    ...
    // MÉTODOS de construcción (de tu código original)
    ...
    // MÉTODOS de precios y alquileres (de tu código original)
